...
from auth_app.src.core.config import logstash_data
...
class AppTagFilter(logging.Filter):
    """Фильтр для добавления тега auth_app в запись лога."""

    def filter(self, record):
        record.tags = ["auth_app"]
        return True
...
LOGGING = {
    ...
    "handlers": {
        ...
        "file": {
            ...
            "filename": "/var/log/auth_app/auth_app.log",
